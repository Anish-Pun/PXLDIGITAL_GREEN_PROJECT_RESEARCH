...
void printGetTemps(float temperatureC, float temperatureF);
...
void loop()
{
    ...
    { // If functie die checkt als de interval tijd  is verstreken
        lastSendMs = now;
        ...
        {                                  // als de locatie gevonden worden door de GPS module dat minder oud is dan 2 seconde
            lat = gps.location.lat();      // Latitude van de huidige locatie
            lon = gps.location.lng();      // longitude van de huidige locatie
            alt = gps.altitude.meters();   // altitude in meters
            sats = gps.satellites.value(); // hoeveel heden gevonden satelieten
        }
        else
        {
        ...
        snprintf(buf, sizeof(buf),
                 "{\"device\":\"Anish-Device\",\"lat\":%.6f,\"lon\":%.6f,\"alt\":%.2f,\"sats\":%d,\"tempC\":%.2f,\"tempF\":%.2f,\"ts\":%lu}",
                 lat, lon, alt, sats, temperatureC, temperatureF, now);

        String payload = String(buf);
        Serial.println(payload);                               // JSON printing for logging
        ResponseStatus s = e22ttl.sendMessage(payload + "\n"); // newline-delimited for easy parsing on receive side
        Serial.print("Result: ");
        Serial.println(s.getResponseDescription());
        printGetTemps(temperatureC, temperatureF);              // printen van temperatuur readings
